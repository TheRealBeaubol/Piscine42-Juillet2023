...
/*   Updated: 2023/07/27 19:15:01 by lboiteux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <stdlib.h>

int	char_finder(char character, char *charset);

int	ft_count_malloc(char *str, char *sep)
{
	int	num_sep;
	int	i;
	int	verification;
...
char	*ft_strdup(char *src, int start, int end)
{
	...
	dest = malloc(end - start * sizeof(*dest));
	if (dest == NULL)
		return (dest);
	i = start;
	j = 0;
...
void	ft_split_in_table(char **table, char *str, char *charset)
{
	int	i;
	int	start;
	int	end;
	...
	while (str[i])
	{
			...
			&& start < i && start != -1)
		{
			...
			table[tablei++] = ft_strdup(str, start, end);
			start = -1;
		}
		i++;
	}
	if (start != -1)
		table[tablei++] = ft_strdup(str, start, i);
	table[tablei] = 0;
}
...
char	**ft_split(char *str, char *charset)
{
	...
	if (table == 0)
		return (0);
	ft_split_in_table(table, str, charset);
	return (table);
}
